...
const COLOR_SETTING_RESOURCE : Resource = preload\
		("res://addons/Space Generator/UI/color_setting.tscn")
...
func upload_palette():
	...
	if OS.get_name() != "Web" or javascript_utility == null:
		push_error("Palette upload only available in web builds")
		return
		
	var new_palette_image : Image = await javascript_utility.load_image()
